...
Visualizing routing congestion and wirelength patterns is a key early step in the physical design process. Our group devised a congestion map — heatmap of routing demand vs capacity across the chip) to illustrate where the design is over-stressed. EDA tools can divide the chip area into a grid of global routing cells (GRCs) and color code each cell by its routing overflow — the red tone color here indicates regions where the number of required routing tracks exceed what’s available. The congestion heat map provides a spatial insight that allows our group to see hotspots that need attention. In practice, our mentors mention these maps can highlight issues such as too many cells being bunched near a macro pin or a channel with multiple long nets running through. In the floorplanning and placement stage of the chip process, the congestion heatmap gives us knowledge to which areas the cells need to be spread out more or reordering logic to reduce the number of crossings in that channel. 
Analyzing congestion at an early stage is important because moving a macro to alleviate a choke point is much easier to do before detailed routing has already been made. harder in the later stages of the design process. Thus, a congestion heatmap aids in a quicker design closure because it mitigates risks in the later stages of the design process. Our group made a congestion heatmap for one of the chip designs to pinpoint where in the layout wires are most crowded, which serve as guides to improve floorplanning (e.g. changing block placement or adding routing layers).


<img src="images/heatmap.png"/>

Demand is the amount of routing resources that need to pass through a GRC, and is a target in congestion prediction tasks. Our group visualized routing demand distribution – the number of routing tracks necessary in each region of the chip — with a histogram. The large Superblue circuits’ demand can be visualized as demand on the x-axis and frequency on the axis. The shape of the distribution reflects the overall routability and balance of the design. In the Superblue 18 circuit, demand seems to be bimodal with most demand concentrated at 3 and 17.  This particular chip’s histogram is also narrow and somewhere centered in one of the peaks, which indicates the routing load is balanced across the chip, i.e. most GRCs are clustered around moderate demand. In contrast, our group found some of the other chips’ histogram followed a wide or skewed distribution, which indicates uneven routing resource usage or bad design.

<img src="images/demand.png"/>


A goal of plotting the demand distribution across the Superblue circuits was to identify outlier regions, which correspond to congestion hotspots on a chip. Our mentors’ domain knowledge helped our group understand that a long tail in the demand histogram indicates a small subset of GRCs where demand supersedes capacity. While plotting the distribution across the chips, our group looked for heavy tails in each histogram for those high demand bins. That would highlight congestion prone areas to look out for. In practice, one of the domain experts explained that physical design tools would report metrics like maximum overflow (a single worse GRC with demand overcapacity) and the percentage of GRCs with overflow.


Plotting this histogram reveals how routing resources are utilized across the design, identifying patterns and outliers. On the Superblue circuits, many histograms have small fraction of regions with sizable demand; these regions have disproportionately large routing load. Our group wished to quantify how spread out demand is across the chips, and in turn scout out how many near-capacity regions there are.
Our group mainly focused on predicting net demand as a target variable, and plotting the demand distribution as part of the EDA process allows us to look through these histograms and identify congestion prone regions. In practical terms, this analysis can serve as a warning for designers to see if the current placement is likely to cause routing congestion before detailed routing occurs. This sort of risk mitigation occurs because if the histogram shows many bins at high demand, engineers can revisit placement to spread out cells or adjust floorplanning like adding extra routing layers/resources. Since detailed routing is a time consuming process, the demand histograms can potentially save time and money by scouting out design flaws before engineers proceed in the next chip design process. 

### Baseline Model

In the second quarter of the capstone our group continued our study of the DE-HNN model architecture and used our learnings to intuit the pros and cons of this architecture. The DE-HNN (Directional Equivariant Hypergraph Neural Network) is a framework for learning circuit properties from netlist graphs. The baseline model implemented by the authors can be characterized as a message-passing neural network on a directed hypergraph representing the Superblue circuit netlist. The dataset depicts the logic gates (nodes) and the wires connecting them (edges), which intuitively leads to a graph representation. Moreover, the Superblue dataset is represented as a hypergraph, nodes correspond to circuit cells (logic gates or blocks), and nets are modeled as hyperedges that connect one driver cell to multiple sink cells​. In general, graph designs representing the chips can vary greatly across companies and types of chips (modem chips, memory chips, etc.), motivating various GNN architectures that account for different types of dependencies and structures. 

Our work was benchmarked against, and later built upon, a two layer DE-HNN model - a GNN architecture that, importantly, represents the design as a directed hypergraph and uses virtual nodes to capture long range interactions between cells. 

<img src="images/baseline.png"/>

Given by the README for the netlist dataset, all cells (nodes) and nets (hyperedge) are associated with feature vectors. Node features contain cell type, dimensions, degree, local connectivity information such as degree distribution, top-10 Laplacian Eigenvector, and persistent diagram features. While each net’s feature is aggregated from its pins (degree).
All the baseline models our group ran include virtual nodes, but this advanced feature can be switched off by setting it to false in the paper implementation. The baseline without virtual nodes simply uses a message passing scheme between cells and nets. In each layer, a cell node gathers messages from the nets it is connected to, and a net (hyperedge) gathers messages from the cells that belong to it. The baseline model our group benchmarked against includes virtual nodes, with validation and test RMSE converging at around 7. In the DE-HNN paper, the targets for prediction include per-net wirelength, per-net demand, and congestion class. Recall that wirelength refers to the post-rout wirelength of that net, demand refers to the total routing demand that net contributes, and congestion is a classification problem of predicting 1 or 0 – whether the region around a cell is congested or not. The central focus of the baseline model and our group’s work is to minimize error in these predictions. As our group discovered, feature elements like capacity, architectural changes, etc. can be added or modified to reach these goals.
...
<img src="images/ablation_dehnn.png"/>

When we ran the ablation study on the DE-HNN model we found that removing features had almost no effect on the performance of the model, with the maximum change being +1.8% from removing eigenvectors. Because of this we thought that model may only be learning from the graph structure itself, not the actual features. To test this, we tried randomizing all of the features to noise, and testing the performance of the model again. 

<img src="images/noise.png"/>

Since by randomizing all of the features the model performed only 0.2% worse than with the features, we found that the DE-HNN only learns on the structure of the graph and not the features of each node and net. However we still want the feature importances through an ablation study, so we also used a random forest, since it can only use the features and not the connections. 
...
The SHAP summary plot for the net features (Figure (ADD FIGURE NUMBER)) also highlights the importance of the eigenvectors, which have a significantly larger impact than the degree.

<img src="images/shap_net.png"/>
...
One of the mentors suggested we can explore different architectural models as a learning process and to see if those can achieve similar results to the baseline model — this road let us discover some flaws and limitations of graph neural networks (GNNs). This way we can compare the limitations and pros of each architecture. Our group then went on a journey to thoroughly understand various GNN architectures to intuit which other architectures will give improvement. The core idea of a GNN is to learn how to represent each node in the graph by considering its own features, the features of its neighbors, and the structure of the graph. The GNN does this through a process called message passing or neighborhood aggregation. Essentially, each node “passes” its features to its neighbors, and each node “receives” messages from its neighbors. These messages are then combined to update the node’s representation. This process is repeated layer by layer to allow information to propagate through the graph.
One of the mentors suggested we can explore different architectural models to see if we can achieve similar results to the baseline model, meaning it doesn’t have to be graph neural networks. This way we can compare the limitations and pros of each architecture. Our group then went on a journey to thoroughly understand various GNN architectures to intuit which other architectures will give improvement. The core idea of a GNN is to learn how to represent each node in the graph by considering its own features, the features of its neighbors, and the structure of the graph. The GNN does this through a process called message passing or neighborhood aggregation. Essentially, each node “passes” its features to its neighbors, and each node “receives” messages from its neighbors. These messages are then combined to update the node’s representation. This process is repeated layer by layer to allow information to propagate through the graph. The DE-HNN model architecture the authors proposed is a very sophisticated model, especially when using virtual nodes to capture long-range interactions. I analyzed a less sophisticated model, namely, the Graph Attention Transformer (GAT) to see what it’s doing differently than DE-HNN. Following our group project’s timeline cadence, these discoveries would later give me insights to use hypervectors. It seems the fundamental difference lies in how a particular model handles the “message passing” step. The GAT requires the concept of “attention” to update the node’s representation. This assumes all neighbors are equally important, which is not true in our case or real-world graphs. We know some neighbors are more important from their persistent diagram, i.e. the topology the node lives in contains richer information than other topologies.
