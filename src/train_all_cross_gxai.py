...
# ---------------------------
# New: Import GraphXAI components
try:
    import matplotlib.pyplot as plt
    from graphxai.explain import Explainer  # Might need a GNNExplainer
    
    # from graphxai.visualizer import plot_feature_importance
except ImportError:
    print("Find right explanation visualization")
# ---------------------------
...
visualize_explanation = True  # set True for GraphXAI explanation visualization

if test:
    restart = True

model_type = "dehnn"  # this can be one of ["dehnn", "dehnn_att", "digcn", "digat"]
...
# GraphXAI explanation visualization
if visualize_explanation:
    try:
        # Select a sample for explanation:
        # Here, we choose one sample from the test set if in test mode, else from validation
        sample_data = h_dataset[all_test_indices[0]] if test else h_dataset[all_valid_indices[0]]
        # Extract targets from the variant data (assumes first tuple)
        target_node = sample_data.variant_data_lst[0][0]  # e.g., cell-based congestion (node demand)
        target_net = sample_data.variant_data_lst[0][2]   # e.g., net-based demand
        model.eval()
        
        # Create an explainer instance for your model (using a gradient-based method as an example)
        explainer = Explainer(model=model, method='grad')  # adjust parameters as needed
        
        # Obtain explanation for node features (cell/node based)
        # The API is hypothetical; assume it returns a dict with a key 'feature_importance'
        node_explanation = explainer.explain_graph(sample_data, target='node', ground_truth=target_node)
        
        # Similarly, obtain explanation for net features
        net_explanation = explainer.explain_graph(sample_data, target='net', ground_truth=target_net)
        
        # Visualize node feature importance
        plt.figure(figsize=(10,6))
        plt.bar(range(len(node_explanation['feature_importance'])), node_explanation['feature_importance'])
        plt.xlabel('Node Feature Index')
        plt.ylabel('Importance')
        plt.title('Node Feature Importance (Cell-based Congestion)')
        plt.show()
        
        # Visualize net feature importance
        plt.figure(figsize=(10,6))
        plt.bar(range(len(net_explanation['feature_importance'])), net_explanation['feature_importance'])
        plt.xlabel('Net Feature Index')
        plt.ylabel('Importance')
        plt.title('Net Feature Importance (Net-based Demand)')
        plt.show()
        
    except Exception as e:
        print("An error occurred during GraphXAI explanation visualization:", e)
# ---------------------------
